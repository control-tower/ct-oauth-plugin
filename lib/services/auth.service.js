  ...
  class AuthService {
    ...
    static async createToken(user, saveInUser) {
      try {
        ...
        const userData = await UserModel.findById(user.id).populate('roles.application');
        let token = null;
    ...
    static async createUserWithoutConfirmation(data) {
      const salt = bcrypt.genSaltSync();
    ...
    static async getRenewModel(token) {
      ...
      const renew = await RenewModel.findOne({
        token
      });
      return renew;
    }

    static async sendResetMail(email, application) {
    ...
    static async checkRevokedToken(ctx, token) {
      ...
      try {
        debug('token', token);
        const user = await WhiteListModel.findOne({
          token
        });
