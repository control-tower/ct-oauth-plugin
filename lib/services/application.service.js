    ...
    class ApplicationService {
        ...
        static async getApplications(user) {
            if (Utils.isCTAdmin(user)) {
                return await ApplicationModel.find().select('-__v').populate('owner', 'email _id').select('-_id');
            }
            debug('User', JSON.stringify(user));
            const userDB = await UserModel.findById(user.id).populate('roles.application', '-owner -_id -__v');
            return userDB.roles.map((role) => Object.assign({}, role.application._doc, { role: role.role }));
        }

        static async getApplicationBySlug(user, appSlug) {
            if (Utils.isCTAdmin(user)) {
                return await ApplicationModel.findOne({ slug: appSlug }).select('-__v').populate('owner', 'email _id');
            }

            const userDB = await UserModel.findById(user.id).populate('roles.application');

            const app = userDB.roles.filter((role) => role.application.slug === appSlug).map((role) => Object.assign({}, role.application._doc, { role: role.role }));
            if (app && app.length > 0) {
                return app[0];
            }
            return {};
        ...
        static async getUsersOfApplication(appSlug) {
            const application = await ApplicationModel.findOne({ slug: appSlug });
            if (!application) {
                throw new ApplicationNotFound(`Application ${appSlug} not found`);
            }
            return UserModel.find({ 'roles.application': application._id }).select('-__v -password -salt -userToken');
        }
        ...
        static async associateApplication(app, email, role) {
            const application = await ApplicationModel.findOne({ slug: app });
            if (!application) {
