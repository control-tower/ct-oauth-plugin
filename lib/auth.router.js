...
module.exports = (plugin, connection, generalConfig) => {
    ...
    const API = (function api() {
        ...
        const localCallback = async ctx => await passport.authenticate('local', (user) => {
            if (!user) {
            ...
            if (ctx.request.type === 'application/json') {
                ...
                let baseUrl = '/auth/success';
                if (ctx.request.query && Object.keys(ctx.request.query).length > 0) {
                    baseUrl = `${baseUrl}?${queryString.stringify(ctx.request.query)}`;
                }
        ...
        async function getUsers(ctx) {
            ...
            ctx.body = await AuthService.getUsers(apps);
        }
    ...
    ApiRouter.get('/success', setCallbackUrl, API.success);
