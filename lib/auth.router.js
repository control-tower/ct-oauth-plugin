...
module.exports = (plugin, connection, generalConfig) => {
  ...
  const API = (function api() {
    ...
    async function success(ctx) {
      ...
      debug('Application', ctx.session.application);
      const brand = await ApplicationService.getApplicationBrandBySlug(ctx.session.application);
      ctx.session.callbackUrl = null;
      ctx.session.generateToken = null;
    ...
    async function logout(ctx) {
      ctx.logout();
      const application = ctx.query.application || 'control-tower';
      ctx.redirect(`/auth?application=${application}`);
    }
    ...
    async function loginView(ctx) {
      ...
      const brand = await ApplicationService.getApplicationBrandBySlug(ctx.query.application);
      const thirdParty = {
        twitter: plugin.config.twitter.active,
        google: plugin.config.google.active,
        facebook: plugin.config.facebook.active,
    ...
    async function redirectLogin(ctx) {
      const application = ctx.query.application || 'control-tower';
      ctx.session.application = application;
      ctx.redirect(`/auth/login?application=${application}`);
    }
    ...
    async function sendResetMail(ctx) {
      ...
      const renew = await AuthService.sendResetMail(ctx.request.body.email, ctx.session.application);
      if (!renew) {
        await ctx.render('request-mail-reset', {
    ...
    async function updateApplications(ctx) {
      try {
        if (ctx.session && ctx.session.application) {
          let user = getUser(ctx);
          if (user.role === 'USER') {
            user = await AuthService.updateApplicationsUser(user.id, [ctx.session.application]);
          }
  ...
  async function setCallbackUrl(ctx, next) {
    ...
    if (!ctx.session.application && ctx.query.application) {
      ctx.session.application = ctx.query.application;
    }
    if (!ctx.session.generateToken) {
