...
module.exports = (plugin, connection, generalConfig) => {
    ...
    const API = (function api() {
        ...
        async function createUser(ctx) {
            ...
            if ((!user && body.role !== 'USER')) {
                debug('Not User and new user is not USER');
                ctx.throw(403, 'Forbidden');
                return;
            } else if ((user.role === 'USER' && body.role !== 'USER')) {
                debug('User is user but the new user is MANAGER or ADMIN');
                ctx.throw(403, 'Forbidden');
                return;
            } else if (user.role === 'MANAGER' && body.role === 'ADMIN') {
            ...
            ctx.request.body.extraUserData = {
                apps: ['data4sdgs']
            };
    ...
    ApiRouter.get('/sign-up', API.getSignUp);
    ApiRouter.post('/sign-up', API.signUp);
